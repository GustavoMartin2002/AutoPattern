...
class SecurityLogger:
  ...
  def log_path_traversal_attempt(self, client_ip: str, attempted_path: str):
    ...
    self.log_event(
      ...
      {"client_ip": client_ip, "attempted_path": attempted_path},
      severity="CRITICAL",
    )
  ...
  def log_validation_error(self, error_type: str, details: str, client_ip: str):
    ...
    self.log_event(
      "validation_error",
      {"error_type": error_type, "details": details, "client_ip": client_ip},
      severity="WARNING",
    )

  def log_exception(
    self, exception_type: str, message: str, client_ip: str | None = None
  ):
