...
from core.domain.exceptions import InvalidPathError, FileSaveError, FileCollisionError
...
class StorageAdapter(IStorage):
    ...
    def save(self, name: str, content: bytes) -> str:
        ...
        # Se 'name' já contém caminho (\ ou /), usa como está
        # Caso contrário, salva em 'outputs/'
        if "\\" in name or "/" in name:
            target_path = name
        else:
            target_path = os.path.join("outputs", name)
        ...
        if directory:
            try:
                ...
                raise InvalidPathError(f"Falha ao criar diretório '{directory}': {str(e)}")
        ...
        try:
            ...
            raise FileCollisionError(f"Erro ao resolver conflito de nomes para '{target_path}': {str(e)}")
        ...
        try:
            ...
            raise FileSaveError(f"Falha ao escrever arquivo em '{final_path}': {str(e)}")
    ...
    def _handle_collision(self, path: str) -> str:
        ...
        while True:
            ...
            if counter > 10000:
                raise FileCollisionError("Limite de tentativas de resolução de nome excedido.")