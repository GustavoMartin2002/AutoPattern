...
class XmlAdapter(IXmlParser):
  ...
  def validate(self, file: File) -> bool:
    ...
    try:
      ...
      raise InvalidXMLError(
        "O arquivo enviado não é um XML válido. Verifique a sintaxe."
      ) from e
  ...
  def extract(
    ...
    if tags:
      ...
      file_tags_cleaned = {
        self.__clean_tag(elem.tag).lower() for elem in root.iter()
      }
      ...
      if missing_tags:
        raise TagsNotFoundError(
          f"As seguintes tags não foram encontradas no arquivo: {', '.join(missing_tags)}"
        )
    ...
    if not tags:
      if not repeated_elements:
        ...
        alerts.append(
          "INFO: Nenhum padrão repetido encontrado. Processando como registro único."
        )
      else:
    ...
    # MODO BUSCA AVANÇADA (TAGS ESPECIFICADAS)
      ...
      context_data = {}
      if repeated_elements:
        context_data = self._extract_hierarchical(
          root, priority_tags, exclude_tag=repeated_tag
        )
    ...
    for elem in elements_to_process:
      ...
      if elem_data:
        ...
        if tags:
          structural_selected = (
            any(st in tags_lower for st in structural_tags)
            if "structural_tags" in locals()
            else False
          )
    ...
    if total_tags > 500:
      alerts.append(
        f"INFO: Grande volume de dados processados ({total_tags} tags)"
      )
  ...
  def _build_result(
    self, items, total, filled, empty, priority, groups, alerts
  ) -> dict[str, Any]:
    """Auxiliar para formatar o dicionário de retorno padrão."""
    return {
      "items": items,
      "hierarchical_data": items,
      "statistics": {"total": total, "preenchidas": filled, "vazias": empty},
  ...
  def _extract_hierarchical(
    ...
    if children:
      ...
      for child in children:
        ...
        new_prefix = (
          f"{current_prefix}_{child_tag}" if current_prefix else child_tag
        )
        ...
        if len(child) == 0:
          ...
          sub_data = self._extract_hierarchical(
            child, priority_tags, exclude_tag, current_prefix=new_prefix
          )
          result.update(sub_data)
    else:
      # Elemento não tem filhos: extrai texto direto (caso base)
      tag_name = (
        f"{current_prefix}_{tag_cleaned}" if current_prefix else tag_cleaned
      )
  ...
  def _group_similar_tags(
    ...
    # Agrupa por prefixo
    for key in all_keys:
      # Tag com underscore: usa parte antes do _, senão usa a tag completa
