...
class ExceptionAdapter:
    ...
    def to_http_response(exc: Exception) -> Tuple[int, Dict[str, Any]]:
        ...
        try:
            ...
            logging.error(f"Exception: {type(exc).__name__}: {str(exc)}")

        # Domain exceptions - seguro de expor
        if isinstance(exc, TagsNotFoundError):
            return 422, {"message": str(exc), "type": "TagsNotFoundError"}

        if isinstance(exc, (InvalidFormatError, InvalidXMLError, InvalidPathError, ValueError)):
            error_type = type(exc).__name__
            message = str(exc) if not is_production else "Erro de validação nos dados fornecidos."
            return 400, {"message": message, "type": error_type}

        if isinstance(exc, DomainError):
            message = str(exc) if not is_production else "Erro no processamento da requisição."
            return 400, {"message": message, "type": type(exc).__name__}

        # Internal errors - sanitiza em produção
        if isinstance(exc, (FileSaveError, ReportGenerationError, FileCollisionError)):
            if is_production:
                return 500, {"message": "Erro no processamento do arquivo.", "type": "ProcessingError"}
            else:
                return 500, {"message": str(exc), "type": type(exc).__name__}

        # Unknown errors - nunca expor detalhes em produção
        if is_production:
            return 500, {"message": "Erro interno do servidor", "type": "InternalError"}
        else:
            return 500, {"message": str(exc), "type": type(exc).__name__}
