...
class ReportAdapter(IReportGenerator):
    ...
    def _prepare_chart_data(self, labels: Any, values: Any) -> Tuple[List[str], List[float]]:
    ...
    def generate_excel(self, data: Dict[str, Any]) -> bytes:
        ...
        try:
            ...
            raise ReportGenerationError(f"Erro ao gerar relatório Excel: {str(e)}")
    ...
    def generate_pdf(self, data: Dict[str, Any]) -> bytes:
        ...
        try:
            ...
            stats = data.get('statistics', {'total': 0, 'preenchidas': 0, 'vazias': 0})
            stats_data = [
                ['Total de Tags', 'Tags Preenchidas', 'Tags Vazias'],
                [str(stats.get('total', 0)), str(stats.get('preenchidas', 0)), str(stats.get('vazias', 0))]
            ]
            ...
            if grupos:
                ...
                for grupo_nome, tags in list(grupos.items())[:5]:
                    ...
                    grupo_data = [[
                        Paragraph(tag['nome'], tag_name_style),
                        Paragraph(str(tag['valor'])[:50] + '...', tag_value_style) if len(str(tag['valor'])) > 50
                    ...
                    if len(tags) > 10:
                        grupo_data.append([
                            Paragraph(f"<i>... e mais {len(tags) - 10} itens</i>", styles['Normal']),
                            Paragraph("<i>(consulte o Excel para detalhes completos)</i>", styles['Normal'])
                        ])
            ...
            raise ReportGenerationError(f"Erro ao gerar relatório PDF: {str(e)}")