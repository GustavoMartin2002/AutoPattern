...
class TestUseFileCompleteFlow:
  ...
  async def test_process_file_success(
    self, simple_xml, temp_output_dir, mock_notifier
  ):
    """
    ...
    use_file = UseFile(
      xml_adapter, report_adapter, storage_adapter, mock_notifier
    )

    # Cria entidade File
    content = simple_xml.encode("utf-8")
    file = File(
      name="test.xml", content=content, size=len(content), extension="xml"
    )

    # Cria opções de processamento
    options = ProcessingOptions(
      formats=["xlsx", "pdf"], output_path=temp_output_dir
    )
    ...
    for file_path in result["generated_files"]:
      assert Path(file_path).exists()  # noqa: ASYNC240
  ...
  @pytest.mark.asyncio
  async def test_process_file_with_specific_tags(
    self, simple_xml, temp_output_dir, mock_notifier
  ):
    ...
    options = ProcessingOptions(
      tags=["NAME", "AGE"], formats=["xlsx"], output_path=temp_output_dir
    )
...
class TestUseFileErrorHandling:
  ...
  @pytest.mark.asyncio
  async def test_invalid_xml_error(
    self, invalid_xml, temp_output_dir, mock_notifier
  ):
    """
  ...
  @pytest.mark.asyncio
  async def test_tags_not_found_error(
    self, simple_xml, temp_output_dir, mock_notifier
  ):
    ...
    options = ProcessingOptions(
      tags=["NONEXISTENT"], formats=["xlsx"], output_path=temp_output_dir
    )
  ...
  async def test_invalid_format_error(
    ...
    UseFile(xml_adapter, report_adapter, storage_adapter, mock_notifier)

    content = simple_xml.encode("utf-8")
    File(name="test.xml", content=content, size=len(content), extension="xml")

    # Deve falhar durante validação de opções
    with pytest.raises(ValueError):
      ProcessingOptions(formats=["invalid"], output_path=temp_output_dir)
  ...
  async def test_invalid_file_extension(
    ...
    content = simple_xml.encode("utf-8")
    file = File(
      name="test.txt", content=content, size=len(content), extension="txt"
    )
...
class TestUseFileProgressNotifications:
  ...
  @pytest.mark.asyncio
  async def test_progress_notifications_sent(
    self, simple_xml, temp_output_dir, mock_notifier
  ):
    """
